...
function variant(id: string, size: string, price: string, compareAt?: string) {
	return {
		node: {
			id,
			title: `${size}`,
...
const sizes = ['US 6', 'US 6.5', 'US 7', 'US 7.5', 'US 8', 'US 8.5', 'US 9', 'US 9.5', 'US 10', 'US 10.5', 'US 11', 'US 12', 'US 13', 'US 14', 'US 15', 'US 16', 'US 17', 'US 18'];
...
function variants(basePrice: string, salePrice?: string) {
	...
	for (const s of sizes) {
		...
		const compareAt = salePrice ? basePrice : undefined;
		edges.push(variant(`v-${s.replace(/\s+/g, '-')}`, s, price, compareAt));
	}
	return { edges };
}
...
function images(handle: string) {
	const basename = handle.replace(/-light|-30|-3|-4$/, '');
	return {
		edges: [
			{ node: { url: `/images/shoes/${basename}.svg`, altText: `${handle} - main view`, width: 800, height: 800 } },
			{ node: { url: `/images/shoes/${basename}-angle.svg`, altText: `${handle} - angle view`, width: 800, height: 800 } },
			{ node: { url: `/images/shoes/${basename}-top.svg`, altText: `${handle} - top view`, width: 800, height: 800 } },
			{ node: { url: `/images/shoes/${basename}-sole.svg`, altText: `${handle} - sole view`, width: 800, height: 800 } },
			{ node: { url: `/images/shoes/${basename}-heel.svg`, altText: `${handle} - heel view`, width: 800, height: 800 } },
			{ node: { url: `/images/shoes/${basename}-detail.svg`, altText: `${handle} - detail view`, width: 800, height: 800 } }
		]
	};
}
...
export const products: Product[] = [
	{
		...
		images: images('nike-air-max-pulse'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('150.00', '120.00'),
	...
	},
	{
		...
		images: images('adidas-ultraboost'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('190.00', '160.00'),
		collections: collection('running', 'Running'),
	...
	},
	{
		...
		images: images('new-balance-990v6'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('200.00'),
	...
	},
	{
		...
		images: images('asics-gel-kayano'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('160.00', '135.00'),
		collections: collection('running', 'Running'),
	...
	},
	{
		...
		images: images('on-cloudmonster'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('170.00'),
		collections: collection('running', 'Running'),
	...
	},
	{
		...
		images: images('puma-velocity-nitro'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('130.00', '110.00'),
		collections: collection('running', 'Running'),
	...
	},
	{
		...
		images: images('hoka-clifton-9'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('145.00', '125.00'),
		collections: collection('running', 'Running'),
	...
	},
	{
		...
		images: images('saucony-endorphin-speed'),
		options: [
			{ name: 'Size', values: sizes }
		],
		variants: variants('170.00'),
